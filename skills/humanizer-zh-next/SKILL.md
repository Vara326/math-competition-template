...
version: 1.3.0
description: |
  去除中文文本中由人工智能生成的痕迹。在编辑或审阅通用中文，以及论文、学位论文、审稿回复、
  基金申请等中文学术文本时使用，使文字更自然，同时保留事实、学术语体、论断与证据的关系。
  本技能基于 op7418/humanizer-zh、blader/humanizer 与 AIScientists-Dev/academic-humanizer，
  遵循“改写而非删除、等量覆盖”原则；学术体裁须加载专门 reference。
license: MIT
...
1. **判断体裁并加载规则** —— 先使用下方矩阵；学术文本必须读取专门 reference。
2. **识别 AI 模式** —— 扫描下列各类模式，并应用相应体裁的例外。
3. **改写，而不是删除** —— 用自然表达替换 AI 腔，并覆盖原文涵盖的全部内容。原文有五段，改写也应有五段。
4. **保留原意** —— 保持核心信息不变。
5. **匹配声音** —— 贴合目标语气（正式、随意、技术专业）。只在内容和作者声音需要时才加个性（见「个性与灵魂」）。
...
文本属于论文、学位论文、摘要、审稿回复、rebuttal、基金申请，或用户明确要求保持学术严谨、校准论断与证据时，**必须先完整阅读并应用 [中文学术写作模式](references/academic-writing.md)**。不要为普通博客、营销、个人叙事、商务说明、法律或百科文本加载该 reference。
...
- **学术材料采用更严格的保真边界。** 数字、实验结果、公式、符号、图表编号、参考文献和 cite key 不得擅自修改；证据不足时收窄论断或标记给作者处理，不能补造证据。
...
3. **没有样本时**，回退到默认：自然、参差、有观点的声音（见「个性与灵魂」）；若文本本身偏中性，就用清楚、具体、克制的现代中文。学术文本按 [中文学术写作模式](references/academic-writing.md) 校准，不使用个人化回退风格。
...
1. **通读原文。** 根据“体裁决策矩阵”判断文本类型和目标读者，**标出所有命中的模式**，并确认该体裁是否允许新增叙事细节。命中学术模式时，先完整读取学术 reference，再继续处理。
2. **写初稿。** 先处理空泛、虚高、模板化的地方，再处理句式、标点、标题和填充词。检查初稿：朗读是否通顺，句长有没有变化，是不是优先用了具体细节和简单判断句（能用“是”就用“是”），语域是否合适，信息是否与原文等量覆盖。个人叙事中如有新增内容，写作时同步记录，不要等到最后凭印象回想；学术文本不得新增事实或证据。
...
**学术模式例外：** 默认交付终稿、简短变更报告和保真确认；完整审计仍在内部完成，只有用户要求查看过程时才展示。具体格式和“需作者处理”的使用条件见 [中文学术写作模式](references/academic-writing.md)。
...
- `AIScientists-Dev/academic-humanizer` commit `94b88b23703bed7df507acae7d6d5876209a0cdf`，见 [中文学术写作模式](references/academic-writing.md)
